...
if vim.g.vscode then
  ...
  map("n", "<leader>be", function()
    action("workbench.action.closeEditorsInOtherGroups")
  end, { desc = "关闭其他组编辑器" })
  map("n", "<leader>bl", function()
    action("workbench.action.toggleEditorGroupLock")
  end, { desc = "切换编辑器组锁定" })
  ...
  -- 文件与搜索操作
  map("n", "-", function()
    action("extension.showDirectoryFiles")
  end, { desc = "显示目录文件" })
  ...
  map("n", "<leader>sg", function()
    action("workbench.action.quickOpen", { args = { "%" } })
  end, { desc = "全局搜索" })
  ...
  map("n", "[h", function()
    action("editor.action.dirtydiff.previous")
  end, { desc = "上一个差异" })
  map("n", "]h", function()
    action("editor.action.dirtydiff.next")
  end, { desc = "下一个差异" })
  map("n", "[t", function()
    action("todo-tree.goToPrevious")
  end, { desc = "上一个TODO" })
  map("n", "]t", function()
    action("todo-tree.goToNext")
  end, { desc = "下一个TODO" })

  -- 代码导航与查询
  map("n", "gh", function()
    action("editor.action.showDefinitionPreviewHover")
  end, { desc = "显示定义预览" })
  map("n", "gl", function()
  ...
  -- 文本格式化与转换
  map("n", "\\c", function()
    action("editor.action.transformToCamelcase")
  end, { desc = "转换为驼峰命名" })
  map("n", "\\p", function()
    action("extension.changeCase.pascal")
  end, { desc = "转换为帕斯卡命名" })
  map("n", "\\s", function()
    action("editor.action.transformToSnakecase")
  end, { desc = "转换为蛇形命名" })
  map("n", "\\t", function()
    action("editor.action.transformToTitlecase")
  end, { desc = "转换为标题命名" })
  map("n", "\\u", function()
  ...
  map("n", "<leader>nf", function()
    action("fileutils.newFolder")
  end, { desc = "新建文件夹" })
  map("n", "<leader>nd", function()
    action("fileutils.removeFile")
  end, { desc = "删除文件" })
  map("n", "<leader>na", function()
    action("fileutils.newFile")
  end, { desc = "新建文件" })
  map("n", "<leader>nn", function()
    action("fileutils.renameFile")
  end, { desc = "重命名文件" })
  map("n", "<leader>nt", function()
    action("workbench.action.populateFileFromSnippet")
  end, { desc = "从代码片段生成文件" })
  ...
  map("n", "<leader>t", function()
    action("editor.emmet.action.matchTag")
  end, { desc = "匹配标签" })
  map("v", "[[", function()
    action("editor.action.smartSelect.shrink")
  end, { desc = "智能选择收缩" })
  map("v", "]]", function()
  ...
  map("v", "S", function()
    action("surround.with")
  end, { desc = "包围" })
  map("v", "W", function()
    action("editor.emmet.action.wrapWithAbbreviation")
  end, { desc = "Emmet包裹" })

  -- ASCII树生成
  map("v", "t", function()
    action("extension.asciiTreeGeneratorFromText")
  end, { desc = "文本生成ASCII树" })
  map("v", "T", function()
  ...
  map("n", "<leader>r", function()
    action("workbench.action.tasks.reRunTask")
  end, { desc = "重新运行任务" })
